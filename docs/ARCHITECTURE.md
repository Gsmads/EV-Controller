...
Байта синхронизации и поля длины нет (ADR-0024). Кодирование COBS убирает
нулевой байт из данных, поэтому `0x00` служит границей кадра и внутри кадра
встретиться не может ни при каких данных.

Парсер поэтому не конечный автомат: копить до разделителя, раскодировать,
проверить CRC. Состояния «жду длину» и «жду N байт данных» исчезли вместе с
полями, которые их порождали.
...
```c
int16_t util_cobs_encode(const uint8_t *src, uint8_t len, uint8_t *dst, uint16_t cap);
int16_t util_cobs_decode(const uint8_t *src, uint8_t len, uint8_t *dst, uint16_t cap);
```

Кодирование, убирающее байт `0x00` из данных, — обрамление кадров протокола
(ADR-0024). Прибавка: один байт на кадр плюс по одному на каждые полные
254 байта. Разбор допускает `dst == src`: выход всегда короче входа.
