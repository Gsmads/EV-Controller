...
# Пусто: последнее исключение, firmware/PRINT.h, снято вместе с переездом
# постоянного хранения за util_rom (ADR-0021). Новое исключение потребует
# нового ADR — список может только сокращаться.
HEADER_EXCEPTIONS = {}
