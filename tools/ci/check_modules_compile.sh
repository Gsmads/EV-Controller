...
# Модули, привязанные к платформе. Остался один: по ADR-0001 платформенным
# и должен быть ровно hal_atmega328p.cpp. MICRO_UART.cpp ушёл вместе
# с переписанным драйвером UART (ADR-0018), PRINT.cpp и app_debug.cpp —
# вместе с переездом постоянного хранения за util_rom (ADR-0021).
